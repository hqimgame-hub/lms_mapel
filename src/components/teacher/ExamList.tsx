...
export function ExamList({ exams, courseId, isTeacher = false }: ExamListProps) {
    return (
        <div className="grid gap-4">
            {exams.map((exam) => (
                <div key={exam.id} className="bg-white p-6 rounded-[2rem] border border-slate-100 hover:border-emerald-500/30 hover:shadow-xl transition-all flex flex-col md:flex-row justify-between gap-6 group">
                    <div className="flex gap-4">
                        <div className="bg-emerald-50 p-4 rounded-2xl h-fit group-hover:bg-emerald-100 transition-colors">
                            <ClipboardList className="text-emerald-500" size={28} />
                        </div>
                        <div>
                            ...
                            <p className="text-slate-500 text-sm mb-4 font-medium">{exam.description || 'Tidak ada deskripsi'}</p>
            ...
            {exams.length === 0 && (
                <div className="text-center py-20 bg-white rounded-[2.5rem] border border-dashed border-slate-200">
                    ...
                    <h4 className="text-lg font-bold text-slate-800 mb-1">Belum Ada Ujian</h4>
                    <p className="text-slate-400 text-sm max-w-xs mx-auto">Buat link ujian Google Form atau instruksi ujian lainnya untuk siswa.</p>
                </div>
            )}
        </div>
    );
